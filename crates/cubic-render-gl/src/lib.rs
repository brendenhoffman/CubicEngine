...
impl GlRenderer {
    fn make_current(
        ...
        fn make_with_template(
            ...
            // Find a config
            let mut configs =
                unsafe { display.find_configs(template.build()) }.context("find_configs")?;
            let config = configs
                .next()
            ...
            let sattrs =
                SurfaceAttributesBuilder::<WindowSurface>::new().build(window_handle, w, h);
...
impl Renderer for GlRenderer {
    fn new(
        ...
        let display =
            unsafe { Display::new(dh, DisplayApiPreference::Egl) }.context("Display::new(EGL)")?;
